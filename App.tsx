...
interface AuthContextType {
  ...
  signup: (name: string, email: string, whatsapp?: string) => void;
  logout: () => void;
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = (email: string) => {
    const u = dbService.login(email);
    if (u) setUser(u);
    else alert("Usuário não encontrado. Tente se cadastrar!");
  };

  const signup = (name: string, email: string, whatsapp?: string) => {
    const u = dbService.signup(name, email, whatsapp);
    setUser(u);
  };
  ...
  const updatePlan = (plan: PlanType) => {
    if (user) {
      dbService.updatePlan(user.id, plan);
      setUser({ ...user, plan });
    }
  };
